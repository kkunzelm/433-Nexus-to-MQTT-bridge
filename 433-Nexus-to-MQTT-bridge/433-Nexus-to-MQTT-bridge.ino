/* ***************************************************************************************************************
 *  
 *  Important: in case of long MQTT message strings you have to modify PubSubClient.h !
 *  
 *             #define MQTT_MAX_PACKET_SIZE 256
 *  
 *  
 * @file          433-Nexus-to-MQTT-bridge.ino
 * @version 	  0.9
 * @author 	  Karl-Heinz Kunzelmann / most references which I used are mentioned in the code
 * @description

 *   Reads sensor data which are encoded in Nexus format.
 ...
 *   
 *   - This sketch sends the data to an MQTT broker
 ...
 *
 * Hardware: Wemos D1 mini V3.1 -> https://www.wemos.cc/en/latest/d1/d1_mini.html) with Hope's RFM210C-433S1 receiver
 ********************************************************************************************************************
 *
...
#include <ESP8266WiFiMulti.h>        // wifi - access best of multiple access points
...
String qthlocator = "JN58SD";           // JN58SD = München, just an example
                                        ...
                                        // how to determine: https://www.karhukoti.com/Maidenhead-Grid-Square-Locator
...
long interval = 2000;                      // non-block delay, necessary because MCU dequeues fifo quicker than sensor enqueues it
...
static uint8_t receiverPin = 4;   // pin 2 = D2 Arduino/Nano
...
void setup() {
      ...
      // initialize digital pin LED_BUILTIN as an output
       ...
       digitalWrite(LED_BUILTIN, HIGH);   // turn the LED off -> Wemos D1 mini LED is inverted
...
void reconnect() {
       ...
       while (!client.connected()) {
              ...
              // Attempt to connect 
              String clientID =qthlocator+"_"+sensortyp;
...
void sendDataToMQTT(){
       ...
       */
       ...
       uint8_t message_length = message.length()+1;
       if (debug){
           Serial.print("Payload message length [byte]: ");
...
ICACHE_RAM_ATTR  void handleInterrupt() {           // ICACHE_RAM_ATTR  = necessary for ESP8266
